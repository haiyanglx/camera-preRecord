...
struct PoliceMediaCodecSource::Puller : public AHandler {
...
PoliceMediaCodecSource::Puller::Puller(const sp<MediaSource> &source)
    : mSource(source),
      mLooper(new ALooper()),
      mIsAudio(false)
{
...
PoliceMediaCodecSource::Puller::~Puller() {
    mLooper->unregisterHandler(id());
    mLooper->stop();
}

void PoliceMediaCodecSource::Puller::Queue::pushBuffer(MediaBuffer *mbuf) {
    mReadBuffers.push_back(mbuf);
}

bool PoliceMediaCodecSource::Puller::Queue::readBuffer(MediaBuffer **mbuf) {
...
void PoliceMediaCodecSource::Puller::Queue::flush() {
    MediaBuffer *mbuf;
...
bool PoliceMediaCodecSource::Puller::readBuffer(MediaBuffer **mbuf) {
    Mutexed<Queue>::Locked queue(mQueue);
    return queue->readBuffer(mbuf);
}

status_t PoliceMediaCodecSource::Puller::postSynchronouslyAndReturnError(
...
status_t PoliceMediaCodecSource::Puller::start(const sp<MetaData> &meta, const sp<AMessage> &notify) {
...
void PoliceMediaCodecSource::Puller::stop() {
...
void PoliceMediaCodecSource::Puller::interruptSource() {
...
void PoliceMediaCodecSource::Puller::stopSource() {
    sp<AMessage> msg = new AMessage(kWhatStop, this);
    (void)postSynchronouslyAndReturnError(msg);
}

void PoliceMediaCodecSource::Puller::pause() {
    Mutexed<Queue>::Locked queue(mQueue);
    queue->mPaused = true;
}

void PoliceMediaCodecSource::Puller::resume() {
    Mutexed<Queue>::Locked queue(mQueue);
    queue->mPaused = false;
}

void PoliceMediaCodecSource::Puller::schedulePull() {
    (new AMessage(kWhatPull, this))->post();
}

void PoliceMediaCodecSource::Puller::handleEOS() {
...
void PoliceMediaCodecSource::Puller::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatStart:
        {
            sp<RefBase> obj;
...
PoliceMediaCodecSource::Output::Output()
    : mEncoderReachedEOS(false),
      mErrorCode(OK) {
}

// static
sp<PoliceMediaCodecSource> PoliceMediaCodecSource::Create(
        const sp<ALooper> &looper,
        const sp<AMessage> &format,
        const sp<MediaSource> &source,
        const sp<IGraphicBufferConsumer> &consumer,
        uint32_t flags) {
    sp<PoliceMediaCodecSource> mediaSource =
            new PoliceMediaCodecSource(looper, format, source, consumer, flags);
...
status_t PoliceMediaCodecSource::setInputBufferTimeOffset(int64_t timeOffsetUs) {
    sp<AMessage> msg = new AMessage(kWhatSetInputBufferTimeOffset, mReflector);
    msg->setInt64("time-offset-us", timeOffsetUs);
    return postSynchronouslyAndReturnError(msg);
}

int64_t PoliceMediaCodecSource::getFirstSampleSystemTimeUs() {
...
status_t PoliceMediaCodecSource::start(MetaData* params) {
    sp<AMessage> msg = new AMessage(kWhatStart, mReflector);
    msg->setObject("meta", params);
    return postSynchronouslyAndReturnError(msg);
}

status_t PoliceMediaCodecSource::stop() {
    sp<AMessage> msg = new AMessage(kWhatStop, mReflector);
    return postSynchronouslyAndReturnError(msg);
}

status_t PoliceMediaCodecSource::pause() {
    (new AMessage(kWhatPause, mReflector))->post();
    return OK;
}

sp<MetaData> PoliceMediaCodecSource::getFormat() {
    Mutexed<sp<MetaData>>::Locked meta(mMeta);
    return *meta;
}

sp<IGraphicBufferProducer> PoliceMediaCodecSource::getGraphicBufferProducer() {
				...
				if(mIsVideo){
					if(mfirstVideoFlag){
						...
						for (List<MediaBuffer*>::iterator it = output->mBufferQueue.begin();it != output->mBufferQueue.end();) {
							...
							(*it)->meta_data()->findInt32(kKeyIsSyncFrame, &iFrame);
							(*it)->meta_data()->findInt64(kKeyTime, &timestampUs);
							(*it)->meta_data()->findInt32(kKeyNumberID, &numberId);
							if(iFrame > 0){
...
void PoliceMediaCodecSource::signalBufferReturned(MediaBuffer *buffer) {
    buffer->setObserver(0);
    buffer->release();
}

PoliceMediaCodecSource::PoliceMediaCodecSource(
        const sp<ALooper> &looper,
	...
	ALOGE("output mime is '%s'", mime.c_str());

	bool mIsAvc = !strncasecmp( MEDIA_MIMETYPE_VIDEO_AVC,mime.c_str(),9);
    bool mIsHevc = !strncasecmp(MEDIA_MIMETYPE_VIDEO_HEVC,mime.c_str(), 10);
...
status_t PoliceMediaCodecSource::init() {
...
status_t PoliceMediaCodecSource::initEncoder() {
    mReflector = new AHandlerReflector<PoliceMediaCodecSource>(this);
    ...
    for (size_t ix = 0; ix < matchingCodecs.size(); ++ix) {
        ...
        ALOGE("output format is '%s'", mOutputFormat->debugString(0).c_str());
...
void PoliceMediaCodecSource::releaseEncoder() {
    if (mEncoder == NULL) {
        return;
    }
...
status_t PoliceMediaCodecSource::postSynchronouslyAndReturnError(
        const sp<AMessage> &msg) {
    sp<AMessage> response;
    status_t err = msg->postAndAwaitResponse(&response);
...
void PoliceMediaCodecSource::signalEOS(status_t err) {
...
void PoliceMediaCodecSource::suspend() {
    CHECK(mFlags & FLAG_USE_SURFACE_INPUT);
    if (mEncoder != NULL) {
...
void PoliceMediaCodecSource::resume(int64_t skipFramesBeforeUs) {
    CHECK(mFlags & FLAG_USE_SURFACE_INPUT);
    if (mEncoder != NULL) {
        sp<AMessage> params = new AMessage;
...
status_t PoliceMediaCodecSource::feedEncoderInputBuffers() {
    MediaBuffer* mbuf = NULL;
...
status_t PoliceMediaCodecSource::onStart(MetaData *params) {
    if (mStopping) {
...
void PoliceMediaCodecSource::onPause() {
    if (mFlags & FLAG_USE_SURFACE_INPUT) {
        suspend();
    } else {
        CHECK(mPuller != NULL);
...
void PoliceMediaCodecSource::setPreRecord(bool value){
	mPreRecord = value;

	if(mPreRecord){
		char prop[92]={0};
	    property_get("persist.sys.precord.duration", prop, "0");
	    int time = atoi(prop);
...
void PoliceMediaCodecSource::setPreRecordDuration(int durationTime){
	if(mPreRecord){
		switch(durationTime){
			case 1:
				mVideoPreRecordMemoryCount = 10 * 25;
				mAudioPreRecordMemoryCount = 10 * 50;
				break;
			case 2:
				mVideoPreRecordMemoryCount = 20 * 25;
				mAudioPreRecordMemoryCount = 20 * 50;
				break;
			case 3:
				mVideoPreRecordMemoryCount = 30 * 25;
				mAudioPreRecordMemoryCount = 30 * 50;
				break;
			default:
				mVideoPreRecordMemoryCount = 10 * 25;
				mAudioPreRecordMemoryCount = 10 * 50;
...
void PoliceMediaCodecSource::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        if (cbID == MediaCodec::CB_INPUT_AVAILABLE) {
            ...
            if (!(flags & MediaCodec::BUFFER_FLAG_CODECCONFIG)) {
				...
				if(mPreRecord){
	            	...
	            	sp<MetaData> meta1 = mFirstBuffer->meta_data();
	            	AVUtils::get()->setDeferRelease(meta1);
	            	mFirstBuffer->setObserver(this);
	            	mFirstBuffer->add_ref();
					mFirstBuffer->meta_data()->setInt32(kKeyIsCodecConfig, true);

					ALOGE("kKeyIsCodecConfig copy");
					mbuf->release();
					mbuf = NULL;
				}
            }
			...
			if(mbuf != NULL){
				...
				//ALOGE("first %02x %02x %02x %02x %02x ",dataStart[0],dataStart[1],dataStart[2],dataStart[3],dataStart[4]);
...

		            {
		            	if(!(flags & MediaCodec::BUFFER_FLAG_CODECCONFIG)){
							...
							if(mIsVideo){
								...
								sizeCount = mAudioPreRecordMemoryCount;
							}

							if(sizeCount > 0){
								if(output->mBufferQueue.size() >= sizeCount){
									
									MediaBuffer *buffer = *output->mBufferQueue.begin();
									ALOGE("delete mBufferQueue.size=%d,source=%s,datasize=%d",(int)(output->mBufferQueue.size()),mIsVideo?"Video":"Audio",(int)(buffer->range_length()));
			        				output->mBufferQueue.erase(output->mBufferQueue.begin());
									buffer->release();
									buffer = NULL;
								...
								ALOGE("there is error occur! sizeCount=%d",sizeCount);
							}

							char prop[92]={0};
	    					property_get("persist.sys.precord.start", prop, "0");
	    					bool mPreRecordStart = (atoi(prop) > 0);
							ALOGV("mPreRecordStart: %d ,sizeCount=%d",mPreRecordStart,sizeCount);
