...
#define LOG_NDEBUG 0
#define LOG_TAG "NativePoliceRecorder"
#include <inttypes.h>
#include <utils/Log.h>

#include <inttypes.h>
#include "WebmWriter.h"
#include "PoliceRecorder.h"
...
#include <media/stagefright/PoliceMediaCodecSource.h>
...
PoliceRecorder::PoliceRecorder(const String16 &opPackageName)
...
sp<IGraphicBufferProducer> PoliceRecorder::querySurfaceMediaSource() const {
    ALOGV("Get SurfaceMediaSource");
    return mGraphicBufferProducer;
}

status_t PoliceRecorder::setAudioSource(audio_source_t as) {
...
status_t PoliceRecorder::setVideoSource(video_source vs) {
...
status_t PoliceRecorder::setOutputFormat(output_format of) {
...
status_t PoliceRecorder::setAudioEncoder(audio_encoder ae) {
...
status_t PoliceRecorder::setVideoEncoder(video_encoder ve) {
...
status_t PoliceRecorder::setVideoSize(int width, int height) {
...
status_t PoliceRecorder::setVideoFrameRate(int frames_per_second) {
...
status_t PoliceRecorder::setCamera(const sp<hardware::ICamera> &camera,
...
status_t PoliceRecorder::setInputSurface(
...
status_t PoliceRecorder::setParamAudioSamplingRate(int32_t sampleRate) {
...
status_t PoliceRecorder::setParamAudioNumberOfChannels(int32_t channels) {
...
status_t PoliceRecorder::setParamAudioEncodingBitRate(int32_t bitRate) {
...
status_t PoliceRecorder::setParamVideoEncodingBitRate(int32_t bitRate) {
...
status_t PoliceRecorder::setParamVideoRotation(int32_t degrees) {
...
status_t PoliceRecorder::setParamMaxFileDurationUs(int64_t timeUs) {
...
status_t PoliceRecorder::setParamMaxFileSizeBytes(int64_t bytes) {
...
status_t PoliceRecorder::setParamInterleaveDuration(int32_t durationUs) {
...
status_t PoliceRecorder::setParamVideoIFramesInterval(int32_t seconds) {
    ALOGV("setParamVideoIFramesInterval: %d seconds", seconds);
    mIFramesIntervalSec = seconds;
    return OK;
}

status_t PoliceRecorder::setParam64BitFileOffset(bool use64Bit) {
...
status_t PoliceRecorder::setParamVideoCameraId(int32_t cameraId) {
...
status_t PoliceRecorder::setParamTrackTimeStatus(int64_t timeDurationUs) {
...
status_t PoliceRecorder::setParamVideoEncoderProfile(int32_t profile) {
...
status_t PoliceRecorder::setParamVideoEncoderLevel(int32_t level) {
...
status_t PoliceRecorder::setParamMovieTimeScale(int32_t timeScale) {
...
status_t PoliceRecorder::setParamVideoTimeScale(int32_t timeScale) {
...
status_t PoliceRecorder::setParamAudioTimeScale(int32_t timeScale) {
...
status_t PoliceRecorder::setParamCaptureFpsEnable(int32_t captureFpsEnable) {
...
status_t PoliceRecorder::setParamCaptureFps(float fps) {
...
status_t PoliceRecorder::setParamGeoDataLongitude(
...
status_t PoliceRecorder::setParamGeoDataLatitude(
...
status_t PoliceRecorder::setParameter(
...
status_t PoliceRecorder::setParameters(const String8 &params) {
...
status_t PoliceRecorder::setListener(const sp<IMediaRecorderClient> &listener) {
    mListener = listener;

    return OK;
}

status_t PoliceRecorder::setClientName(const String16& clientName) {
    mClientName = clientName;

    return OK;
}

status_t PoliceRecorder::prepareInternal() {
    ALOGV("prepare");
    if (mOutputFd < 0) {
...
sp<PoliceMediaCodecSource> PoliceRecorder::createAudioSource() {
	...
	ALOGE("audio sourceSampleRate=%d,mAudioChannels==%d,mAudioBitRate=%d,outSampleRate=%d",sourceSampleRate,mAudioChannels,mAudioBitRate,mSampleRate);
    ...
    sp<PoliceMediaCodecSource> audioEncoder =
            PoliceMediaCodecSource::Create(mLooper, format, audioSource);
    mAudioSourceNode = audioSource;

    if (audioEncoder == NULL) {
        ALOGE("Failed to create audio encoder");
    }
	//audioEncoder->setEncryptEnable(mEncrypFlag);

    return audioEncoder;
}

status_t PoliceRecorder::setupAACRecording() {
    // FIXME:
	...
	//mWriter->setEncryptEnable(mEncrypFlag);
    return setupRawAudioRecording();
}

status_t PoliceRecorder::setupAMRRecording() {
...
status_t PoliceRecorder::setupRawAudioRecording() {
    ...
    sp<PoliceMediaCodecSource> audioEncoder = createAudioSource();
    if (audioEncoder != NULL) {
        CHECK(mWriter != 0);
...
status_t PoliceRecorder::setupRTPRecording() {
    ...
    sp<PoliceMediaCodecSource> source;
...
status_t PoliceRecorder::setupMPEG2TSRecording() {
    ...
    if (mVideoSource < VIDEO_SOURCE_LIST_END) {
        ...
        sp<PoliceMediaCodecSource> encoder;
...
void PoliceRecorder::clipVideoFrameRate() {
...
void PoliceRecorder::clipVideoBitRate() {
...
void PoliceRecorder::clipVideoFrameWidth() {
...
status_t PoliceRecorder::checkVideoEncoderCapabilities() {
...
void PoliceRecorder::setDefaultProfileIfNecessary() {
...
void PoliceRecorder::setDefaultVideoEncoderIfNecessary() {
...
status_t PoliceRecorder::checkAudioEncoderCapabilities() {
    clipAudioBitRate();
    clipAudioSampleRate();
...
void PoliceRecorder::clipAudioBitRate() {
...
void PoliceRecorder::clipAudioSampleRate() {
...
void PoliceRecorder::clipNumberOfAudioChannels() {
...
void PoliceRecorder::clipVideoFrameHeight() {
...
status_t PoliceRecorder::setupMediaSource(
                      sp<MediaSource> *mediaSource) {
...
status_t PoliceRecorder::setupCameraSource(
        sp<CameraSource> *cameraSource) {
    status_t err = OK;
...
status_t PoliceRecorder::setupVideoEncoder(
    ...
    if (cameraSource != NULL) {
		...
		ALOGE("cameraSource colorFormat value: %d", colorFormat);
    ...
    if (tsLayers > 1) {
        ...
        ALOGV("android._prefer-b-frames = %d )", preferBFrames);
    ...
    uint32_t flags = 0;
    if (cameraSource == NULL) {
        flags |= PoliceMediaCodecSource::FLAG_USE_SURFACE_INPUT;
    } else {
        // require dataspace setup even if not using surface input
        format->setInt32("android._using-recorder", 1);
    }

    sp<PoliceMediaCodecSource> encoder = PoliceMediaCodecSource::Create(
	...
	//encoder->setEncryptEnable(mEncrypFlag);
...
status_t PoliceRecorder::setupAudioEncoder(const sp<MediaWriter>& writer) {
    status_t status = BAD_VALUE;
    if (OK != (status = checkAudioEncoderCapabilities())) {
        return status;
    }
...
status_t PoliceRecorder::setupMPEG4orWEBMRecording() {
    ...
    sp<PoliceMPEG4Writer> mp4writer;
    if (mOutputFormat == OUTPUT_FORMAT_WEBM) {
    ...
    if (mVideoSource < VIDEO_SOURCE_LIST_END) {
        ...
        writer->addSource(encoder);
        mVideoEncoderSource = encoder;
		mVideoEncoderSource->setPreRecord(mPreRecord);
    ...
    if (mOutputFormat != OUTPUT_FORMAT_WEBM) {
        ...
        if (!disableAudio && mAudioSource != AUDIO_SOURCE_CNT) {
			...
			mAudioEncoderSource->setPreRecord(mPreRecord);
        }
...
void PoliceRecorder::setupMPEG4orWEBMMetaData(sp<MetaData> *meta) {
    ...
    if (mOutputFormat != OUTPUT_FORMAT_WEBM) {
		...
		ALOGV("mRotationDegrees=%d",mRotationDegrees);
...
status_t PoliceRecorder::resume() {
    ALOGV("resume");
    if (!mStarted) {
        return INVALID_OPERATION;
    }
...
status_t PoliceRecorder::setEncryptEnable(int flag,int value){
	ALOGE("setEncryptEnable: %d,value=%d", flag,value);
	mEncrypFlag = (flag == 1) ? true:false;
	return OK;
}


status_t PoliceRecorder::getMaxAmplitude(int *max) {
