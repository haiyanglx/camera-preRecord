...
//#define LOG_NDEBUG 0
#define LOG_TAG "PoliceMPEG4Writer"
...
#include <media/stagefright/PoliceMPEG4Writer.h>
...
static unsigned char EncryptKey[33] = {0};


class PoliceMPEG4Writer::Track {
public:
    Track(PoliceMPEG4Writer *owner, const sp<IMediaSource> &source, size_t trackId);
...
private:
    ...
    struct ListTableEntries {
        ...
        void write(PoliceMPEG4Writer *writer) const {
...
PoliceMPEG4Writer::PoliceMPEG4Writer(int fd)
    : mFd(dup(fd)),
...
status_t PoliceMPEG4Writer::Track::dump(
...
// static
const char *PoliceMPEG4Writer::Track::getFourCCForMime(const char *mime) {
    if (mime == NULL) {
        return NULL;
    }
...
status_t PoliceMPEG4Writer::addSource(const sp<IMediaSource> &source) {
...
status_t PoliceMPEG4Writer::startTracks(MetaData *params) {
    if (mTracks.empty()) {
...
void PoliceMPEG4Writer::addDeviceMeta() {
...
int64_t PoliceMPEG4Writer::estimateMoovBoxSize(int32_t bitRate) {
...
status_t PoliceMPEG4Writer::start(MetaData *param) {
    if (mInitCheck != OK) {
        return UNKNOWN_ERROR;
    }

	char prop[92]={0};
    property_get("persist.sys.encrypt.config", prop, "0");
    mEncrptFlag = (atoi(prop) > 0);
	ALOGE("MPEG4Writer mEncryptFlag=%d",mEncrptFlag);

	if(mEncrptFlag){
		memcpy(EncryptKey, CRYPT_KEY, strlen(CRYPT_KEY));
	}
    ...
    ALOGV("luxun movie time scale: %d,mUse4ByteNalLength=%d", mTimeScale,mUse4ByteNalLength);
	...
	ALOGI("luxun mOffset: %" PRId64 " ,mEstimatedMoovBoxSize %" PRId64 " bytes,mStreamableFile=%d,mWriteMoovBoxToMemory=%d",
         mOffset, mEstimatedMoovBoxSize,mStreamableFile,mWriteMoovBoxToMemory);
    ...
    mOffset = mMdatOffset;
	ALOGI("luxun mOffset: %" PRId64 " mMoovExtraSize=%"  PRId64 ",mUse32BitOffset=%d,",
         mOffset, mMoovExtraSize,mUse32BitOffset);
...
bool PoliceMPEG4Writer::use32BitFileOffset() const {
    return mUse32BitOffset;
}

status_t PoliceMPEG4Writer::pause() {
    if (mInitCheck != OK) {
        return OK;
    }
    mPaused = true;
...
void PoliceMPEG4Writer::stopWriterThread() {
 ...
 */
void PoliceMPEG4Writer::writeCompositionMatrix(int degrees) {
...
status_t PoliceMPEG4Writer::reset() {
    if (mInitCheck != OK) {
        return OK;
    } else {
        if (!mWriterThreadStarted ||
    ...
    ALOGI("luxun mOffset: %" PRId64 " mMdatOffset=%ld,mUse32BitOffset=%d,",
         mOffset, mMdatOffset,mUse32BitOffset);
    if (mUse32BitOffset) {
        lseek64(mFd, mMdatOffset, SEEK_SET);
	...
	ALOGI("luxun mOffset: %" PRId64 " maxDurationUs=%"  PRId64 ",mWriteMoovBoxToMemory=%d,",
         mOffset, maxDurationUs,mWriteMoovBoxToMemory);
    ...
    if (mWriteMoovBoxToMemory) {
		...
		ALOGI("luxun mFreeBoxOffset: %" PRId64 " mMoovBoxBufferOffset=%"  PRId64 ",mWriteMoovBoxToMemory=%"  PRId64 ",mWriteMoovBoxToMemory=%d",
         mFreeBoxOffset, mMoovBoxBufferOffset,mEstimatedMoovBoxSize,mWriteMoovBoxToMemory);
...
uint32_t PoliceMPEG4Writer::getMpeg4Time() {
...
void PoliceMPEG4Writer::writeMvhdBox(int64_t durationUs) {
    ...
    ALOGV("luxun writeMvhdBox mTracks.size=%d",(int)(mTracks.size()));
    endBox();  // mvhd
}

void PoliceMPEG4Writer::writeMoovBox(int64_t durationUs) {
    beginBox("moov");
...
void PoliceMPEG4Writer::writeFtypBox(MetaData *param) {
    beginBox("ftyp");
...
void PoliceMPEG4Writer::sendSessionSummary() {
...
status_t PoliceMPEG4Writer::setInterleaveDuration(uint32_t durationUs) {
    mInterleaveDurationUs = durationUs;
    return OK;
}

void PoliceMPEG4Writer::lock() {
    mLock.lock();
}

void PoliceMPEG4Writer::unlock() {
    mLock.unlock();
}

off64_t PoliceMPEG4Writer::addSample_l(MediaBuffer *buffer) {
...
void PoliceMPEG4Writer::StripStartcode(MediaBuffer *buffer) {
...
off64_t PoliceMPEG4Writer::addMultipleLengthPrefixedSamples_l(MediaBuffer *buffer) {
    off64_t old_offset = mOffset;
...
off64_t PoliceMPEG4Writer::addLengthPrefixedSample_l(MediaBuffer *buffer) {
    off64_t old_offset = mOffset;
...
size_t PoliceMPEG4Writer::write(
    ...
    if (mWriteMoovBoxToMemory) {
        ...
        if (moovBoxSize > mEstimatedMoovBoxSize) {
			...
			//ALOGI("luxun moovBoxSize: %" PRId64 " mMoovBoxBufferOffset=%"  PRId64 ",mWriteMoovBoxToMemory=%d,",
         //moovBoxSize, mMoovBoxBufferOffset,mWriteMoovBoxToMemory);
...
void PoliceMPEG4Writer::beginBox(uint32_t id) {
...
void PoliceMPEG4Writer::beginBox(const char *fourcc) {
...
void PoliceMPEG4Writer::writeInt8(int8_t x) {
    write(&x, 1, 1);
}

void PoliceMPEG4Writer::writeInt16(int16_t x) {
    x = htons(x);
    write(&x, 1, 2);
}

void PoliceMPEG4Writer::writeInt32(int32_t x) {
    x = htonl(x);
    write(&x, 1, 4);
}

void PoliceMPEG4Writer::writeInt64(int64_t x) {
    x = hton64(x);
    write(&x, 1, 8);
}

void PoliceMPEG4Writer::writeCString(const char *s) {
    size_t n = strlen(s);
    write(s, 1, n + 1);
}

void PoliceMPEG4Writer::writeFourcc(const char *s) {
    CHECK_EQ(strlen(s), 4);
    write(s, 1, 4);
}
...
void PoliceMPEG4Writer::writeLatitude(int degreex10000) {
...
void PoliceMPEG4Writer::writeLongitude(int degreex10000) {
    bool isNegative = (degreex10000 < 0);
    char sign = isNegative? '-': '+';
 ...
 */
status_t PoliceMPEG4Writer::setGeoData(int latitudex10000, int longitudex10000) {
...
status_t PoliceMPEG4Writer::setCaptureRate(float captureFps) {
    if (captureFps <= 0.0f) {
        return BAD_VALUE;
    }
...
status_t PoliceMPEG4Writer::setTemporalLayerCount(uint32_t layerCount) {
    if (layerCount > 9) {
        return BAD_VALUE;
    }
...
void PoliceMPEG4Writer::write(const void *data, size_t size) {
    write(data, 1, size);
}

bool PoliceMPEG4Writer::isFileStreamable() const {
    return mStreamableFile;
}

bool PoliceMPEG4Writer::exceedsFileSizeLimit() {
    // No limit
    if (mMaxFileSizeLimitBytes == 0) {
        return false;
    }
...
bool PoliceMPEG4Writer::exceedsFileDurationLimit() {
    // No limit
    if (mMaxFileDurationLimitUs == 0) {
        return false;
    }
...
bool PoliceMPEG4Writer::reachedEOS() {
    bool allDone = true;
    for (List<Track *>::iterator it = mTracks.begin();
         it != mTracks.end(); ++it) {
        if (!(*it)->reachedEOS()) {
...
void PoliceMPEG4Writer::setStartTimestampUs(int64_t timeUs) {
...
int64_t PoliceMPEG4Writer::getStartTimestampUs() {
    Mutex::Autolock autoLock(mLock);
    return mStartTimestampUs;
}

size_t PoliceMPEG4Writer::numTracks() {
    Mutex::Autolock autolock(mLock);
    return mTracks.size();
}
...
PoliceMPEG4Writer::Track::Track(
        PoliceMPEG4Writer *owner, const sp<IMediaSource> &source, size_t trackId)
    : mOwner(owner),
...
void PoliceMPEG4Writer::Track::updateTrackSizeEstimate() {
...
void PoliceMPEG4Writer::Track::addOneStscTableEntry(
...
void PoliceMPEG4Writer::Track::addOneStssTableEntry(size_t sampleId) {
	ALOGV("luxun addOneStssTableEntry sampleId=%d", (int)sampleId);
    mStssTableEntries->add(htonl(sampleId));
}

void PoliceMPEG4Writer::Track::addOneSttsTableEntry(
        size_t sampleCount, int32_t duration) {

    if (duration == 0) {
        ALOGW("0-duration samples found: %zu", sampleCount);
    }
	//ALOGW("luxun  addOneSttsTableEntry: sampleCount=%d,duration=%d", (int)sampleCount,duration);
    mSttsTableEntries->add(htonl(sampleCount));
    mSttsTableEntries->add(htonl(duration));
}

void PoliceMPEG4Writer::Track::addOneCttsTableEntry(
        size_t sampleCount, int32_t duration) {

    if (mIsAudio) {
        return;
    }
	ALOGV("luxun addOneCttsTableEntry sampleCount=%d,duration=%d",(int)sampleCount,duration);
    mCttsTableEntries->add(htonl(sampleCount));
    mCttsTableEntries->add(htonl(duration));
}

void PoliceMPEG4Writer::Track::addChunkOffset(off64_t offset) {
...
void PoliceMPEG4Writer::Track::setTimeScale() {
    ALOGV("setTimeScale");
    // Default time scale
    mTimeScale = 90000;
...
void PoliceMPEG4Writer::Track::getCodecSpecificDataFromInputFormatIfPossible() {
    ...
    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)) {
        ...
        if (mMeta->findData(kKeyESDS, &type, &data, &size)) {
			ALOGE("luxun AUDIO ESDS=%d,size=%d", type,(int)size);
            ESDS esds(data, size);
...
void PoliceMPEG4Writer::Track::initTrackingProgressStatus(MetaData *params) {
...
// static
void *PoliceMPEG4Writer::ThreadWrapper(void *me) {
    ALOGV("ThreadWrapper: %p", me);
    PoliceMPEG4Writer *writer = static_cast<PoliceMPEG4Writer *>(me);
    writer->threadFunc();
    return NULL;
}

void PoliceMPEG4Writer::bufferChunk(const Chunk& chunk) {
...
void PoliceMPEG4Writer::writeChunkToFile(Chunk* chunk) {
    ...
    while (!chunk->mSamples.empty()) {
		...
		uint8_t* dataStart = (uint8_t*)buffer->data()+buffer->range_offset();

		if(mEncrptFlag){
			if(chunk->mTrack->isAudio()){
				ALOGE("luxun audio data size=%d,offset=%d",(int)buffer->size(),(int)buffer->range_offset());
				for(int i=0;i<16;i++){
				...
				ALOGE("luxun video data size=%d",(int)buffer->size());
				for(int i=0;i<64;i++){
					ALOGE("%02x %02x %02x %02x",dataStart[i],dataStart[i+1],dataStart[i+2],dataStart[i+3]);
					i=i+3;
				}
...
void PoliceMPEG4Writer::writeAllChunks() {
...
bool PoliceMPEG4Writer::findChunkToWrite(Chunk *chunk) {
...
void PoliceMPEG4Writer::threadFunc() {
    ALOGV("threadFunc");
...
status_t PoliceMPEG4Writer::startWriterThread() {
...
status_t PoliceMPEG4Writer::Track::start(MetaData *params) {
    if (!mDone && mPaused) {
        mPaused = false;
        mResumed = true;
        return OK;
...
status_t PoliceMPEG4Writer::Track::pause() {
    mPaused = true;
    return OK;
}

status_t PoliceMPEG4Writer::Track::stop() {
...
bool PoliceMPEG4Writer::Track::reachedEOS() {
    return mReachedEOS;
}

// static
void *PoliceMPEG4Writer::Track::ThreadWrapper(void *me) {
...
const uint8_t *PoliceMPEG4Writer::Track::parseParamSet(
        const uint8_t *data, size_t length, int type, size_t *paramSetLen) {
...
status_t PoliceMPEG4Writer::Track::copyAVCCodecSpecificData(
...
status_t PoliceMPEG4Writer::Track::copyHEVCCodecSpecificData(
        const uint8_t *data, size_t size) {
...
status_t PoliceMPEG4Writer::Track::copyCodecSpecificData(
	...
	ALOGV("luxun copyCodecSpecificData end ");
	
    return OK;
}

status_t PoliceMPEG4Writer::Track::parseAVCCodecSpecificData(
        const uint8_t *data, size_t size) {
...
status_t PoliceMPEG4Writer::Track::makeAVCCodecSpecificData(
        const uint8_t *data, size_t size) {
    ...
    for (List<AVCParamSet>::iterator it = mSeqParamSets.begin();
		...
		ALOGV("luxun avc data sps=%d",it->mLength);
    ...
    for (List<AVCParamSet>::iterator it = mPicParamSets.begin();
		...
		ALOGV("luxun avc data pps=%d",it->mLength);
        // PPS Nal unit (picture parameter set length bytes)
        memcpy(&header[2], it->mData, picParamSetLength);
        header += (2 + picParamSetLength);
    }

	ALOGV("luxun make avc data start,size=%d",(int)mCodecSpecificDataSize);
	uint8_t * header1 = (uint8_t *)mCodecSpecificData;
	for(int i=0;i<(int)mCodecSpecificDataSize;i++){
		ALOGV("%x \t",header1[i]);
	}
	ALOGV("luxun make avc data end ");
...
status_t PoliceMPEG4Writer::Track::parseHEVCCodecSpecificData(
        const uint8_t *data, size_t size, HevcParameterSets &paramSets) {
...
status_t PoliceMPEG4Writer::Track::makeHEVCCodecSpecificData(
        const uint8_t *data, size_t size) {
 ...
 */
void PoliceMPEG4Writer::Track::updateDriftTime(const sp<MetaData>& meta) {
...
status_t PoliceMPEG4Writer::Track::threadEntry() {
    int32_t count = 0;
	...
	if(!mIsAudio){
		ALOGW("luxun interleaveDurationUs= %" PRId64 "us",interleaveDurationUs);
	}
    ...
    while (!mDone && (err = mSource->read(&buffer)) == OK) {
        ...
        if (AVUtils::get()->canDeferRelease(buffer->meta_data())) {
			...
			//ALOGE("luxun AVUtils canDeferRelease true,avoid cloning the buffer");
        } else {
		...
		if(mOwner->mEncrptFlag){
				...
				if(mIsAvc){
					if(((dataStart[0] == 0x0) && (dataStart[1] == 0x0) &&
						(dataStart[2] == 0x0) && (dataStart[3] == 0x01) &&
						(dataStart[4] == 0x65)) ||((dataStart[0] == 0x0) && 
						(dataStart[1] == 0x0) &&(dataStart[2] == 0x0) && 
						(dataStart[3] == 0x01) &&(dataStart[4] == 0x41))){
						dataStart += 5;
					...
					if(((dataStart[0] == 0x0) && (dataStart[1] == 0x0) &&
						(dataStart[2] == 0x0) && (dataStart[3] == 0x01) &&
						(dataStart[4] == 0x26)) ||((dataStart[0] == 0x0) && 
						(dataStart[1] == 0x0) &&(dataStart[2] == 0x0) && 
						(dataStart[3] == 0x01) &&(dataStart[4] == 0x02))){
						dataStart += 5;
						mOwner->myAes->cipher(EncryptKey, 32, dataStart, 16, dataStart);
					}
				}
...
bool PoliceMPEG4Writer::Track::isTrackMalFormed() const {
    if (mIsMalformed) {
        return true;
    }
...
void PoliceMPEG4Writer::Track::sendTrackSummary(bool hasMultipleTracks) {
...
void PoliceMPEG4Writer::Track::trackProgressStatus(int64_t timeUs, status_t err) {
...
void PoliceMPEG4Writer::trackProgressStatus(
...
void PoliceMPEG4Writer::setDriftTimeUs(int64_t driftTimeUs) {
    ALOGV("setDriftTimeUs: %" PRId64 " us", driftTimeUs);
    Mutex::Autolock autolock(mLock);
    mDriftTimeUs = driftTimeUs;
}

int64_t PoliceMPEG4Writer::getDriftTimeUs() {
    ALOGV("getDriftTimeUs: %" PRId64 " us", mDriftTimeUs);
    Mutex::Autolock autolock(mLock);
    return mDriftTimeUs;
}

bool PoliceMPEG4Writer::isRealTimeRecording() const {
    return mIsRealTimeRecording;
}

bool PoliceMPEG4Writer::useNalLengthFour() {
    return mUse4ByteNalLength;
}

void PoliceMPEG4Writer::Track::bufferChunk(int64_t timestampUs) {
    ALOGV("bufferChunk");
...
int64_t PoliceMPEG4Writer::Track::getDurationUs() const {
    return mTrackDurationUs;
}

int64_t PoliceMPEG4Writer::Track::getEstimatedTrackSizeBytes() const {
    return mEstimatedTrackSizeBytes;
}

status_t PoliceMPEG4Writer::Track::checkCodecSpecificData() const {
    const char *mime;
    CHECK(mMeta->findCString(kKeyMIMEType, &mime));
...
void PoliceMPEG4Writer::Track::writeTrackHeader(bool use32BitOffset) {
...
void PoliceMPEG4Writer::Track::writeStblBox(bool use32BitOffset) {
...
void PoliceMPEG4Writer::Track::writeVideoFourCCBox() {
    const char *mime;
	...
	ALOGV("luxun writeVideoFourCCBox mime=%s,fourcc=%s", mime,fourcc);
	...
	ALOGV("luxun writeVideoFourCCBox width=%d,height=%d", width,height);
...
void PoliceMPEG4Writer::Track::writeColrBox() {
    ...
    if (mMeta->findInt32(kKeyColorPrimaries, (int32_t*)&aspects.mPrimaries)
		...
		ALOGV("luxun writeColrBox primaries=%d,transfer=%d,coeffs=%d,fullRange=%d", primaries,transfer,coeffs,fullRange);
...
void PoliceMPEG4Writer::Track::writeAudioFourCCBox() {
    const char *mime;
    bool success = mMeta->findCString(kKeyMIMEType, &mime);
    CHECK(success);
    const char *fourcc = getFourCCForMime(mime);
	...
	ALOGV("luxun writeAudioFourCCBox mime=%s,fourcc=%s", mime,fourcc);
	...
	ALOGV("luxun writeAudioFourCCBox nChannels=%d,samplerate=%d", nChannels,samplerate);
    CHECK(success);
...
void PoliceMPEG4Writer::Track::writeMp4aEsdsBox() {
	...
	ALOGV("luxun writeMp4aEsdsBox avgBitrate=%d,maxBitrate=%d,mCodecSpecificDataSize=%d", avgBitrate,maxBitrate,(int)mCodecSpecificDataSize);
...
void PoliceMPEG4Writer::Track::writeMp4vEsdsBox() {
    CHECK(mCodecSpecificData);
    CHECK_GT(mCodecSpecificDataSize, 0);
...
void PoliceMPEG4Writer::Track::writeTkhdBox(uint32_t now) {
    ...
    ALOGV("luxun writeTkhdBox mTrackId=%d",mTrackId);
    mOwner->writeInt32(0);             // reserved
    int64_t trakDurationUs = getDurationUs();
	ALOGV("luxun writeTkhdBox getDurationUs=%" PRId64,trakDurationUs);
	...
	ALOGV("luxun writeTkhdBox mRotation=%d", mRotation);
    ...
    if (mIsAudio) {
        ...
        success = success && mMeta->findInt32(kKeyHeight, &height);
        CHECK(success);
		ALOGV("luxun writeTkhdBox width=%d,height=%d", width,height);
...
void PoliceMPEG4Writer::Track::writeVmhdBox() {
...
void PoliceMPEG4Writer::Track::writeSmhdBox() {
...
void PoliceMPEG4Writer::Track::writeHdlrBox() {
...
void PoliceMPEG4Writer::Track::writeMdhdBox(uint32_t now) {
    int64_t trakDurationUs = getDurationUs();
...
void PoliceMPEG4Writer::Track::writeDamrBox() {
...
void PoliceMPEG4Writer::Track::writeUrlBox() {
...
void PoliceMPEG4Writer::Track::writeDrefBox() {
...
void PoliceMPEG4Writer::Track::writeDinfBox() {
    mOwner->beginBox("dinf");
    writeDrefBox();
    mOwner->endBox();  // dinf
}

void PoliceMPEG4Writer::Track::writeAvccBox() {
    CHECK(mCodecSpecificData);
	...
	ALOGV("luxun writeAvccBox mUse4ByteNalLength=%d,mCodecSpecificDataSize=%d", mOwner->mUse4ByteNalLength,(int)mCodecSpecificDataSize);
...
void PoliceMPEG4Writer::Track::writeHvccBox() {
    CHECK(mCodecSpecificData);
    CHECK_GE(mCodecSpecificDataSize, 5);
...
void PoliceMPEG4Writer::Track::writeD263Box() {
...
void PoliceMPEG4Writer::Track::writePaspBox() {
...
int32_t PoliceMPEG4Writer::Track::getStartTimeOffsetScaledTime() const {
...
void PoliceMPEG4Writer::Track::writeCttsBox() {
	...
	ALOGV("luxun writeCttsBox delta=%d", delta);
...
void PoliceMPEG4Writer::Track::writeStssBox() {
...
void PoliceMPEG4Writer::Track::writeStszBox() {
...
void PoliceMPEG4Writer::Track::writeStscBox() {
...
void PoliceMPEG4Writer::Track::writeStcoBox(bool use32BitOffset) {
...
void PoliceMPEG4Writer::writeUdtaBox() {
    beginBox("udta");
    writeGeoDataBox();
    endBox();
}

void PoliceMPEG4Writer::writeHdlr() {
    beginBox("hdlr");
...
void PoliceMPEG4Writer::writeKeys() {
...
void PoliceMPEG4Writer::writeIlst() {
    size_t count = mMetaKeys->countEntries();
...
void PoliceMPEG4Writer::writeMetaBox() {
    size_t count = mMetaKeys->countEntries();
    if (count == 0) {
        return;
    }
 ...
 * Geodata is stored according to ISO-6709 standard.
 */
void PoliceMPEG4Writer::writeGeoDataBox() {
    beginBox("\xA9xyz");
    /*
